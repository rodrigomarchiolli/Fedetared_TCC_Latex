...
\newcommand{\monografiaAno}{01 de Julho 2026}
\newcommand{\monografiaCoordenador}{Prof. Antonio Carlos Sobieranski, Dr.}
\newcommand{\monografiaOrientador}{Prof. Martín Vigil, Dr.}
...
Building on this premise, this study investigates federated learning for privacy-preserving occupational stress analysis through a rapid literature review and a practical implementation. The work addresses limitations of traditional centralized methods by combining local model training with a mobile privacy gateway: physiological data are sent from ESP32-based wearable devices to an authorized smartphone through authenticated BLE bonding, signed with HMAC-SHA256 (Hash-based Message Authentication Code using the SHA-256 hash function), and routed through Tor to local training nodes hosted at healthcare institutions. In this context, Tor hides the network origin of the request, while HMAC-SHA256 verifies message integrity and authorization at the application layer. In the federated workflow, raw signals remain on-premises; only model updates are shared for global aggregation through Federated Averaging (FedAvg), after which the global server redistributes the updated weights to participating clients. This implementation demonstrates the integration of ESP32, BLE, Tor, HMAC, and Flower for privacy-preserving data transmission and local training. The validation has two distinct dimensions: the end-to-end hardware and software architecture was functionally tested, whereas the stress classification model was trained and validated using the public WESAD dataset rather than data collected by the prototype sensors in a live deployment.
...
This research employed a mixed-methods approach, incorporating a rapid literature review and practical implementation to evaluate the effectiveness and security of federated learning for detecting and monitoring occupational stress. 

The rapid literature review followed PRISMA guidelines, ensuring a structured process for identifying, selecting, and synthesizing relevant studies. The rapid literature review aimed to explore the current state of research on federated learning, traditional machine learning, occupational stress, and wearable technologies, providing a comprehensive understanding of the field. 
...
To guide the rapid literature review, the following research questions were established:
\begin{itemize}
...
\begin{longtable}{p{4.5cm} p{3.2cm} p{2.2cm} p{2.8cm}}
\caption{Summary of articles included in the rapid literature review.}
\label{tab:included_articles} \\

\toprule
\textbf{Reference} & \textbf{Focus} & \textbf{ML/FL Algorithm} & \textbf{Physiological Data/Dataset} \\ \midrule
\endfirsthead

\caption[]{Summary of articles included in the rapid literature review. (Continued)} \\
...
\cite{abdulrahman_alahmadi_32bc288c} & Privacy-preserved IoMT stress detection & FL, ANN, RF, k-NN & WESAD (ECG, EDA, EMG, Resp, TEMP) \\ \midrule

\cite{ahmad_almadhor_8cc2e670} & FL-based stress detection with edge computing & FL, DNN & WESAD (EDA) \\ \midrule

\cite{yekta_said_can_3e1184b6} & Privacy-preserving FL for wearable IoT & FL, Deep Learning & Heart activity (smart bands) \\ \midrule

\cite{neha_gahlan_d8b04f5d} & FL inspired privacy sensitive emotion recognition & FL, CNN & Multi-modal physiological sensors \\ \midrule

\cite{jayant_vyas_bcd9860b} & Driver recommendation using FL & FL, Multi-task Learning & UAH-DriveSet, HCI Lab, PhysioNet \\ \midrule

\cite{m__bharathi_b0a198b0} & FL for healthcare IoT: origins and applications & FL, ML & Healthcare IoT \\ \midrule

\cite{mohamed_benouis_35501772} & FL for mental health monitoring & FL, Multi-task Learning & Multimodal physiological signals \\ \midrule

\cite{jessica_chia_liu_cd0e0973} & FL for health data privacy & FL (simulation) & Wearable physiological and motion data \\ \midrule

\cite{nastasha_johnson_93d644ed} & RAYYAN systematic review tool & N/A & N/A \\ \midrule

\cite{adam_sadilek_c59c73f4} & Privacy-first health research with FL & FL, Differential Privacy & Multi-site health studies \\ \midrule

\cite{chuan_ma_e07241ba} & Data privacy and security in FL & FL, privacy techniques & Simulated data \\
...
The practical prototype consolidates six operational layers, as summarized in Table~\ref{tab:architecture_layers}. Compared with the initial design, the most significant change is the insertion of a mobile application between the ESP32-based sensing unit and the local training node hosted by the healthcare institution. Rather than requiring each embedded device to establish an anonymized connection, physiological windows are first received by the smartphone, which then performs secure forwarding through the Tor network toward the institution's local server.
...
IoMT Devices & ESP32 test boards & Replay of pre-recorded WESAD windows through BLE & Device pseudonyms; authenticated BLE bonding; single-owner access control \\ \midrule
Mobile Gateway & React Native application & Reception of ESP32 windows and secure forwarding & Local secure storage; Tor client on the smartphone \\ \midrule
...
\begin{enumerate}[leftmargin=*]
\item The ESP32 test boards replay pre-recorded WESAD physiological windows and transmit them to the authorized mobile device through an authenticated Bluetooth Low Energy bond.
...
The physical setup shown in Figure~\ref{fig:physical_setup} consists of two ESP32 development boards connected to the notebook by USB, the mobile application acting as the BLE central device, and the notebook running the Dockerized services used by the prototype. The USB connections provide power to the ESP32 boards and support firmware/fixture loading. In the current evaluation, the ESP32 does not capture live physiological readings from sensors. Instead, pre-recorded WESAD signal windows are loaded into the microcontroller's FLASH memory. Every 30 seconds, a new window is read from FLASH into a RAM buffer, simulating periodic sensor sampling. When the smartphone requests data over BLE, the buffered window is transmitted to the mobile gateway. Therefore, this setup validates the ESP32-to-mobile communication, buffering, BLE transmission, mobile ingestion path, and local Dockerized execution environment, not live physiological data acquisition.

\begin{figure}[htbp]
\centering
\safeincludegraphics[width=0.8\textwidth]{imagens/architeture/setup_fisico.png}
\caption{Physical prototype setup with two ESP32 development boards connected to the notebook by USB, the mobile application open on the device screen, and the notebook running the Dockerized prototype services. The setup validates BLE communication and mobile ingestion using pre-recorded WESAD windows, not live sensor acquisition.}
\label{fig:physical_setup}
\end{figure}

Figure~\ref{fig:architecture} provides a visual overview of the operational layers and their interactions.

\begin{figure}[H]
\centering
\safeincludegraphics[width=0.85\textwidth]{imagens/architeture/Evolved System Architecture.png}
\caption{Evolved IoMT architecture for privacy-preserving occupational stress monitoring. In the current prototype, pre-recorded physiological windows flow from the ESP32 sensing unit to the owner-authorized mobile gateway through authenticated Bluetooth Low Energy bonding, then traverse Tor with HMAC-authenticated requests, and are processed locally at the healthcare institution before federated model aggregation.}
...
The authentication contract\footnote{In software and web development, a contract is the agreed interface between components, such as required fields, payload schemas, endpoints, authentication rules, and expected responses.} operates as follows. The local training node generates a QR code containing two fields: \texttt{address}, corresponding to the \texttt{.onion} hidden service URL, and \texttt{password}, a shared secret known only to the institution and the authorized mobile client. When submitting data, the mobile application computes:
...
\item \textbf{Controlled transmission tests:} the mobile-to-Tor transmission path was validated with controlled test payloads rather than continuous ESP32 physiological streams.

\item \textbf{Absence of multi-institutional benchmark:} although the local Flower client and global Flower server components define the federated execution path, the present evaluation does not report a multi-institutional experiment with independent hospital nodes and heterogeneous local distributions.
...
\item \textbf{Mobile gateway as single point of failure:} the current architecture routes all physiological data through the mobile application. If the gateway device goes offline or loses network connectivity, data transmission to the local training node is interrupted, creating an availability limitation not mitigated by the current prototype.

\end{enumerate}

\section{Discussion of the Implications Discovered}

The practical results reinforce the central thesis of this research: occupational stress monitoring can be pursued through a layered privacy architecture that combines federated learning with complementary anonymization and authentication mechanisms. Five implications emerge from the implementation.
...
Fifth, the scope clarifications above carry concrete methodological consequences. The stress detection validated in this work corresponds to general physiological arousal under controlled conditions, not to occupational stressors encountered in real work environments. Extrapolating these results to occupational stress monitoring therefore requires caution, because the current validation does not cover workplace settings. Additionally, the cross-subject aggregated training approach, while practical for initial validation, does not account for the inter-individual variability of physiological stress responses. The federated architecture proposed in this project is, however, inherently compatible with personalization strategies: each client could perform local fine-tuning on subject-specific data, improving the adequacy of stress thresholds for individual users.
...
Future work should extend the present evaluation by completing live ESP32 stream integration, performing subject-level cross-validation, characterizing end-to-end latency under Tor routing, and conducting multi-institutional federated training rounds with independent hospital nodes and heterogeneous local distributions. Additional directions include investigating personalization strategies such as per-subject local fine-tuning within the federated client \cite{ereno2025avaliando}, as well as fault-tolerant gateway designs such as ESP32-side store-and-forward mechanisms or multi-gateway redundancy.
...
BVP, EDA, TEMP, Resp & Physiological signals related to blood volume pulse, electrodermal activity, skin temperature, and respiration. & Basis for stress-related feature extraction and classification. In the WESAD benchmark, these signals were collected with wearable devices under a controlled laboratory protocol; in the prototype, pre-recorded WESAD windows are stored in the ESP32 FLASH memory and transmitted via BLE to simulate sensor acquisition. \\ \midrule
...
HMAC-SHA256 & Cryptographic message authentication mechanism based on a shared secret and SHA-256 hashing. & Verifies request authenticity and integrity before physiological data are accepted by the local API. It was selected as a lightweight symmetric authentication mechanism, avoiding the higher computational overhead of asymmetric cryptographic signatures in resource-constrained IoT-style communication while preserving message integrity and authenticity \cite{okpu2026iothealthcare}. \\ \midrule
