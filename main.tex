...
\usepackage{enumitem}
\usepackage{placeins}
% HIGHLIGHTING COLORS & COMMANDS DEFINED BY ANTIGRAVITY
\usepackage{xcolor}
\definecolor{COR_ADICIONADO}{HTML}{385723} % dark green text
\definecolor{COR_ALTERADO}{HTML}{C65911}   % dark orange text
\newcommand{\adicionado}[1]{{\color{COR_ADICIONADO}#1}}
\newcommand{\alterado}[1]{{\color{COR_ALTERADO}#1}}
\usepackage{pdfpages}
\usepackage{textcase}
...
\alterado{Articles were selected based on well-defined inclusion criteria as described in Table~\ref{tab:criterios}, such as peer-reviewed status, publication date (recent ten years), focus on federated learning, stress monitoring, and wearable technology. Articles that did not meet these criteria or lacked sufficient methodological detail were excluded. The selection process involved title and abstract screening, followed by full-text evaluation.}
...
\alterado{The search was conducted in Web of Science, Scopus, PubMed, and IEEE Xplore databases using the following search string to ensure relevance: ("occupational stress" OR "workplace stress" OR stress) AND "federated learning". The specific search strings and adjustments for each database were as follows:}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item
\alterado{        Search String: \texttt{ALL=(("occupational stress" OR "workplace stress" OR stress) AND "federated learning")}}
        \item
    ...
    \begin{itemize}
        \item
\alterado{        Search String: \texttt{TITLE-ABS-KEY ( ( "occupational stress" OR "workplace stress" OR stress ) AND "federated learning" )}}
        \item
        Adjustments: Include only the document type \textit{Article}
        \item
    ...
    \begin{itemize}
        \item
\alterado{        Search String: \texttt{("occupational stress"[Title/Abstract] OR "workplace stress"[Title/Abstract] OR stress[Title/Abstract]) AND "federated learning"[Title/Abstract]}}
        \item
...
\alterado{Beyond the mentioned techniques, there is growing interest in using more complex algorithms, such as Convolutional Neural Networks (CNN) and Recurrent Neural Networks (RNN), specifically designed for analyzing sequential and temporal data acquired by wearable sensors \cite{yekta_said_can_3e1184b6} [Direta]. Hybrid techniques and Multi-task Learning (MTL) methods have also been proposed to enhance federated model accuracy and robustness by sharing representations across related tasks\cite{mohamed_benouis_35501772} [Parcial].}

\alterado{Ensemble learning techniques, like Gradient Boosting Machines and AdaBoost, are also highlighted for significantly improving predictive performance by combining multiple individual models, thus enhancing generalization capability in heterogeneous federated data scenarios \cite{alaa_abd-alrazaq_e52622}.}
...
\toprule
\alterado{\textbf{Algorithm}} & \alterado{\textbf{Type}} & \alterado{\textbf{Reported Performance}} & \alterado{\textbf{Noise Robustness}} & \alterado{\textbf{Computational Cost}} & \alterado{\textbf{Suitability for FL}} & \alterado{\textbf{Reference}} \\ \midrule
\alterado{ANN}                & \alterado{Non-linear}    & \alterado{Varies}             & \alterado{Medium}                    & \alterado{High}                        & \alterado{High}                        & \alterado{\cite{alaa_abd-alrazaq_e52622}} \\ \midrule
\alterado{Random Forest}      & \alterado{Non-linear}    & \alterado{Varies}             & \alterado{High}                      & \alterado{Medium-High}                 & \alterado{High}                        & \alterado{\cite{adam_sadilek_c59c73f4}} \\ \midrule
\alterado{SVM}                & \alterado{Linear/Non-linear} & \alterado{Varies}            & \alterado{Medium}                    & \alterado{Medium-High}                 & \alterado{Medium}                      & \alterado{\cite{hao_huang_8976046f}} \\ \midrule
\alterado{k-NN}               & \alterado{Non-linear}    & \alterado{Varies}             & \alterado{Low}                       & \alterado{Medium-Low}                  & \alterado{Medium}                      & \alterado{\cite{alaa_abd-alrazaq_e52622}} \\ \midrule
\alterado{Logistic Regression} & \alterado{Linear}       & \alterado{Varies}             & \alterado{Low-Medium}                & \alterado{Low}                         & \alterado{High}                        & \alterado{\cite{alaa_abd-alrazaq_e52622}} \\ 
\bottomrule
\end{tabular}%
}
\end{table}
...
\alterado{Additionally, the effectiveness of these technologies depends on their ability to accurately capture and interpret physiological signals indicative of stress \cite{ahmad_almadhor_8cc2e670} [Direta].}
...
\alterado{To make the results self-contained, the glossary in Appendix~A (Table~\ref{tab:technical_terms_results}) summarizes the main technical terms used throughout this section and their role in the implemented prototype.}
...
\alterado{In the proposed workflow, a single ESP32 simulates the collection of pre-recorded physiological signals from the WESAD dataset, reading them from FLASH memory and transmitting them via BLE to the mobile gateway. Wrist or pulse sensors provide BVP, EDA, and skin temperature measurements. BVP, or blood volume pulse, captures changes in blood flow and is related to cardiovascular response. EDA, or electrodermal activity, captures changes in skin conductance and is commonly associated with sympathetic nervous system activation. The chest sensor provides respiratory data through a strain gauge, which detects expansion and contraction during breathing. The ESP32 transmits these physiological readings to the mobile application through authenticated Bluetooth Low Energy bonding. The smartphone then acts as a privacy gateway between the local sensing layer and the hospital-side training infrastructure.}

The mobile gateway pipeline comprises three main stages:

\begin{enumerate}[leftmargin=*]
\item
\textbf{Credential provisioning:} the local training node API generates a QR code containing a JSON object with the \texttt{.onion} service address and a shared password. The user scans this QR code with the mobile application, and the credentials are stored securely on the device.
\item
\alterado{\textbf{Local request preparation:} the application receives simulated physiological data from the ESP32, serializes the readings into a JSON payload, computes an HMAC-SHA256 signature using the shared password, and attaches the resulting digest to the request through the \texttt{x-signature} header.}
\item
\textbf{Secure submission and validation:} the application sends a \texttt{POST} request to \texttt{/send-pairs} through the Tor network. The hospital API verifies the HMAC signature and persists valid data locally.
\end{enumerate}
...
\alterado{IoMT Devices} & \alterado{ESP32 and wearable sensors} & \alterado{Simulated physiological data transmission} & \alterado{Device pseudonyms; authenticated BLE bonding; single-owner access control} \\ \midrule
...
\begin{enumerate}[leftmargin=*]
\item
\alterado{Wearable sensors connected to a single ESP32 relay pre-stored physiological windows and transmit them to the authorized mobile device through an authenticated Bluetooth Low Energy bond.}
\item
The mobile application stores configuration credentials locally and prepares a signed HTTP request.
\item
The request traverses the Tor network and reaches the hidden service exposed by the local training node.
\item
The local API validates the HMAC signature and persists the received data in the institution's database.
\item
During a federated training round, the local Flower client reads these stored records, trains locally, and returns only updated model parameters to the global server.
\end{enumerate}

\adicionado{The ESP32 does not capture live physiological readings from sensors. Instead, pre-recorded WESAD signal windows are stored in the microcontroller's FLASH memory. Every 30 seconds, a new window is read from FLASH into a RAM buffer, simulating real sensor sampling. When the smartphone requests data over BLE, the entire buffer is flushed to the mobile gateway. This pipeline, shown in Figure~\ref{fig:physical_setup}, validates the transmission and processing chain, not the physiological data acquisition itself.}
...
\alterado{Figure~\ref{fig:architecture} provides a visual overview of these layers and their interactions. Figure~\ref{fig:federated_integration_architecture} illustrates the complete multi-institutional integration.}
...
\alterado{The Tor network \cite{dingledine2004tor} provides \textit{topological anonymity} by routing traffic through a sequence of relay nodes using onion encryption. In practical terms, this means that the destination service does not directly observe the real IP address of the mobile device. In this architecture, Tor complements, but does not replace, transport-layer protections such as TLS. Its primary role is to obscure the network origin of the mobile gateway before data reach the local training node, thereby reducing the risk of correlating a participant's IP address with a specific healthcare institution or individual session.}
...
\adicionado{The middleware also validates the schema of the POST body, checking that all required fields are present and correctly typed. This prevents man-in-the-middle attacks from injecting malformed payloads even when the HMAC signature itself is valid.}
...
\alterado{\footnotesize}
\alterado{\renewcommand{\arraystretch}{1.05}}
\resizebox{\textwidth}{!}{%
\begin{tabular}{
  >{\raggedright\arraybackslash\setlength{\parindent}{0pt}}p{2.5cm}
  >{\raggedright\arraybackslash\setlength{\parindent}{0pt}}p{2.3cm}
  >{\raggedright\arraybackslash\setlength{\parindent}{0pt}}p{3.8cm}
  >{\raggedright\arraybackslash\setlength{\parindent}{0pt}}p{2.0cm}
  >{\raggedright\arraybackslash\setlength{\parindent}{0pt}}p{3.4cm}
}
\toprule
\alterado{\textbf{Aspect}} & \alterado{\textbf{Baseline MLP}} & \alterado{\textbf{Proposed TabularMLP}} & \alterado{\textbf{Value}} & \alterado{\textbf{Purpose}} \\ \midrule
\alterado{Hidden layers} & \alterado{Two FC layers} & \alterado{Three FC (128, 128, 64)} & \alterado{—} & \alterado{—} \\ \midrule
\alterado{Activation} & \alterado{ReLU} & \alterado{GELU \cite{hendrycks2016gelu}} & \alterado{—} & \alterado{—} \\ \midrule
\alterado{Normalization} & \alterado{Not used} & \alterado{BatchNorm1d \cite{ioffe2015batch}} & \alterado{—} & \alterado{—} \\ \midrule
\alterado{Regularization} & \alterado{Limited} & \alterado{Dropout \cite{srivastava2014dropout} + AdamW weight decay} & \alterado{wd=$10^{-3}$, drop=0.25} & \alterado{Prevents overfitting in small dataset} \\ \midrule
\alterado{Output} & \alterado{3 class logits} & \alterado{3 class logits} & \alterado{—} & \alterado{—} \\ \cmidrule{1-5}
\alterado{Optimizer} & \alterado{—} & \alterado{AdamW \cite{loshchilov2019decoupled}} & \alterado{$2\times10^{-3}$} & \alterado{Adaptive optimization with decoupled weight regularization} \\ \midrule
\alterado{Batch size} & \alterado{—} & \alterado{64} & \alterado{64} & \alterado{Samples per optimization step} \\ \midrule
\alterado{Max epochs} & \alterado{—} & \alterado{300} & \alterado{300} & \alterado{Upper bound on training passes} \\ \midrule
\alterado{LR scheduling} & \alterado{—} & \alterado{Reduce on plateau} & \alterado{factor 0.5, patience 15} & \alterado{Refines when macro F1 stagnates} \\ \midrule
\alterado{Early stopping} & \alterado{—} & \alterado{Patience 50} & \alterado{50 epochs} & \alterado{Stops after no improvement} \\ \midrule
\alterado{Gradient clipping} & \alterado{—} & \alterado{Norm 1.0} & \alterado{1.0} & \alterado{Prevents unstable gradient updates} \\ \bottomrule
\end{tabular}%
}
\end{table}
...
\alterado{The scaled models use online Gaussian noise augmentation \cite{goodfellow2016deep} after standardization. In this setting, a noise value such as $0.03$ has the same meaning for all features because the scaler places them in approximately comparable units.}
...
\alterado{The \textit{New full} configuration achieved the highest local validation macro F1-score, 0.9565 at epoch 77, outperforming the original baseline score of 0.9241 by approximately 3.2 percentage points. Its validation loss \cite{goodfellow2016deep} was also the lowest among the evaluated models (0.1454), and its overfitting gap was small (0.0145, see Figure~\ref{fig:wesad_overfit}). This indicates that the regularized TabularMLP improved local generalization compared with the simpler baseline, whose overfitting gap was 0.0728.}
...
\alterado{Five limitations warrant explicit acknowledgment:}

\begin{enumerate}[leftmargin=*]
\item
\textbf{Window-level split:} the WESAD train/validation split was performed at the window level rather than the subject level, meaning windows from the same participant may appear in both partitions. This may moderately inflate validation metrics relative to true out-of-subject generalization.

\item
\textbf{No-scaler performance trade-off:} the selected no-scaler model achieved lower local validation performance than the best scaled model. This was an intentional engineering trade-off: removing \texttt{StandardScaler} reduced local macro F1-score but avoided a preprocessing dependency that was not reliably reproducible across embedded and simulated data paths, local training clients, server validation, and mobile inference.

\item
\textbf{Controlled transmission tests:} the mobile-to-Tor transmission path was validated with controlled test payloads. Live integration with continuous ESP32 physiological streams remains an ongoing deployment step.

\item
\textbf{No multi-institutional benchmark yet:} although the local Flower client and global Flower server components define the federated execution path, the present evaluation does not yet report a multi-institutional experiment with independent hospital nodes and heterogeneous local distributions.

\item
\adicionado{\textbf{Absence of personalization:} the model was trained in an aggregated cross-subject manner without individual calibration or fine-tuning. Since physiological stress thresholds (e.g., EDA, heart rate variability, temperature) vary across individuals, a one-size-fits-all model may not correctly capture the stress/non-stress boundary for each person. Future work should investigate personalization strategies such as per-subject local fine-tuning within the federated client, which is architecturally compatible with the proposed federated framework \cite{ereno2025avaliando}.}

\item
\adicionado{\textbf{Mobile gateway as single point of failure:} the current architecture routes all physiological data through the mobile application. If the gateway device goes offline or loses network connectivity, data transmission to the local training node is interrupted. Future work should investigate fault-tolerant designs, such as store-and-forward mechanisms on the ESP32 side or multi-gateway redundancy, to mitigate this limitation.}
