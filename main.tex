...
\usepackage{xcolor}
\definecolor{COR_ADICIONADO}{HTML}{E2F0D9} % soft green
\definecolor{COR_ALTERADO}{HTML}{FFF2CC}   % soft orange
\newcommand{\adicionado}[1]{\colorbox{COR_ADICIONADO}{#1}}
\newcommand{\alterado}[1]{\colorbox{COR_ALTERADO}{#1}}
\newcommand{\adicionadopar}[1]{\noindent\colorbox{COR_ADICIONADO}{\parbox{\dimexpr\linewidth-2\fboxsep}{#1}}}
\newcommand{\alteradopar}[1]{\noindent\colorbox{COR_ALTERADO}{\parbox{\dimexpr\linewidth-2\fboxsep}{#1}}}
...
\alteradopar{Articles were selected based on well-defined inclusion criteria as described in Table~\ref{tab:criterios}, such as peer-reviewed status, publication date (recent ten years), focus on federated learning, stress monitoring, and wearable technology. Articles that did not meet these criteria or lacked sufficient methodological detail were excluded. The selection process involved title and abstract screening, followed by full-text evaluation.}
...
\alteradopar{The search was conducted in Web of Science, Scopus, PubMed, and IEEE Xplore databases using the following search string to ensure relevance: ("occupational stress" OR "workplace stress" OR stress) AND "federated learning". The specific search strings and adjustments for each database were as follows:}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item
\alteradopar{        Search String: \texttt{ALL=(("occupational stress" OR "workplace stress" OR stress) AND "federated learning")}}
        \item
    ...
    \begin{itemize}
        \item
\alteradopar{        Search String: \texttt{TITLE-ABS-KEY ( ( "occupational stress" OR "workplace stress" OR stress ) AND "federated learning" )}}
        \item
    ...
    \begin{itemize}
        \item
\alteradopar{        Search String: \texttt{("occupational stress"[Title/Abstract] OR "workplace stress"[Title/Abstract] OR stress[Title/Abstract]) AND "federated learning"[Title/Abstract]}}
        \item
...
\alteradopar{Additionally, the effectiveness of these technologies depends on their ability to accurately capture and interpret physiological signals indicative of stress \cite{ahmad_almadhor_8cc2e670} [Direta].}
...
\alteradopar{To make the results self-contained, the glossary in Appendix~A (Table~\ref{tab:technical_terms_results}) summarizes the main technical terms used throughout this section and their role in the implemented prototype.}
...
\alteradopar{In the proposed workflow, a single ESP32 simulates the collection of pre-recorded physiological signals from the WESAD dataset, reading them from FLASH memory and transmitting them via BLE to the mobile gateway. Wrist or pulse sensors provide BVP, EDA, and skin temperature measurements. BVP, or blood volume pulse, captures changes in blood flow and is related to cardiovascular response. EDA, or electrodermal activity, captures changes in skin conductance and is commonly associated with sympathetic nervous system activation. The chest sensor provides respiratory data through a strain gauge, which detects expansion and contraction during breathing. The ESP32 transmits these physiological readings to the mobile application through authenticated Bluetooth Low Energy bonding. The smartphone then acts as a privacy gateway between the local sensing layer and the hospital-side training infrastructure.}
...
\item
\alteradopar{\textbf{Local request preparation:} the application receives simulated physiological data from the ESP32, serializes the readings into a JSON payload, computes an HMAC-SHA256 signature using the shared password, and attaches the resulting digest to the request through the \texttt{x-signature} header.}
\item
...
\begin{enumerate}[leftmargin=*]
\item
\alteradopar{Wearable sensors connected to a single ESP32 relay pre-stored physiological windows and transmit them to the authorized mobile device through an authenticated Bluetooth Low Energy bond.}
\item
...
\alteradopar{Figure~\ref{fig:architecture} provides a visual overview of these layers and their interactions. Figure~\ref{fig:federated_integration_architecture} illustrates the complete multi-institutional integration.}
...
\alteradopar{The Tor network \cite{dingledine2004tor} provides \textit{topological anonymity} by routing traffic through a sequence of relay nodes using onion encryption. In practical terms, this means that the destination service does not directly observe the real IP address of the mobile device. In this architecture, Tor complements, but does not replace, transport-layer protections such as TLS. Its primary role is to obscure the network origin of the mobile gateway before data reach the local training node, thereby reducing the risk of correlating a participant's IP address with a specific healthcare institution or individual session.}
...
\adicionadopar{The middleware also validates the schema of the POST body, checking that all required fields are present and correctly typed. This prevents man-in-the-middle attacks from injecting malformed payloads even when the HMAC signature itself is valid.}
...
\alteradopar{The scaled models use online Gaussian noise augmentation \cite{goodfellow2016deep} after standardization. In this setting, a noise value such as $0.03$ has the same meaning for all features because the scaler places them in approximately comparable units.}
...
\alteradopar{The \textit{New full} configuration achieved the highest local validation macro F1-score, 0.9565 at epoch 77, outperforming the original baseline score of 0.9241 by approximately 3.2 percentage points. Its validation loss \cite{goodfellow2016deep} was also the lowest among the evaluated models (0.1454), and its overfitting gap was small (0.0145, see Figure~\ref{fig:wesad_overfit}). This indicates that the regularized TabularMLP improved local generalization compared with the simpler baseline, whose overfitting gap was 0.0728.}
