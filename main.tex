...
\begin{itemize}
    ...
    \item \textbf{Figure and diagram generation:} The system architecture diagram (Figure~\ref{fig:architecture}) was initially drafted by the authors, and generative AI was used to produce a refined version with improved visual design. The initial versions of the mobile application screen designs were exported from Figma, using Google's Material Design (Stitch) as inspiration for the visual interface. The PRISMA flow diagram (Figure~\ref{fig:prisma}) was generated using the RAYYAN tool \cite{nastasha_johnson_93d644ed}. The experimental results charts were produced using matplotlib. The TabularMLP architecture diagram was produced using TikZ.
\end{itemize}
