...
\begin{tikzpicture}[node distance=1.5cm and 3cm, every node/.style={font=\small}]
  \node[draw, rectangle, rounded corners, minimum width=3cm, minimum height=1.2cm, fill=blue!10, align=center] (wrist) {Wrist/Pulse Sensor\\ (BVP, EDA, TEMP)};
  \node[draw, rectangle, rounded corners, minimum width=3cm, minimum height=1.2cm, fill=green!10, below=of wrist, align=center] (chest) {Chest Sensor\\ (Respiration)};
  \node[draw, rectangle, rounded corners, minimum width=3cm, minimum height=1.2cm, fill=orange!10, right=of wrist, align=center] (esp32) {ESP32\\ Microcontroller};
  
  \draw[-Stealth, thick] (wrist.east) -- node[above, font=\footnotesize] {BLE} (esp32.west);
  \draw[-Stealth, thick] (chest.east) -- node[below, font=\footnotesize] {BLE} (esp32.west);
\end{tikzpicture}
